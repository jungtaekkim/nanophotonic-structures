...
class Combinatorial2D(base_structure.BaseStructure):
    def __init__(
        self,
        depth_pml,
        size_mesh,
    ...
    def define_geometries(self, variables):
        ...
        for ind_block_x in range(0, self.num_x):
            for ind_block_z in range(0, self.num_z):
                current_index = ind_block_x * self.num_z + ind_block_z
                ...
                material_block = mp.Block(
                    size=mp.Vector3(self.size_material_block, self.size_material_block, mp.inf),
                    center=mp.Vector3(center_x, center_z, 0),
...
if __name__ == '__main__':
    ...
    variables = np.random.RandomState(42).choice(12, size=80)
